...
class BSBdataset(Dataset):
    def __init__(self, dataset_path, size=(224, 224)):
        ...
        self.frame_transform = transforms.Compose([transforms.ColorJitter(0.01, 0.01, 0.01, 0),])
        self.all_transform = transforms.Compose([transforms.RandomHorizontalFlip()])
        self.frame_aff_transform = transforms.Compose([transforms.RandomAffine(degrees=15, shear=10, interpolation=InterpolationMode.BILINEAR, fill=(124, 116, 104)),])
        self.gt_aff_transform = transforms.Compose([transforms.RandomAffine(degrees=15, shear=10, interpolation=InterpolationMode.NEAREST, fill=0),])
        self.background_transform = transforms.Compose([
            transforms.ColorJitter(0.01, 0.01, 0.01, 0),
            transforms.RandomResizedCrop(size, scale=(0.8, 1.00), interpolation=InterpolationMode.BILINEAR)])
